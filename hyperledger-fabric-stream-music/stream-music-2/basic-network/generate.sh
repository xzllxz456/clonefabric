...
# generate anchor peer transaction
./bin/configtxgen -profile Channel1 -outputAnchorPeersUpdate ./config/CustomerOrganchors1.tx -channelID $CHANNEL_NAME1 -asOrg CustomerOrg
if [ "$?" -ne 0 ]; then
...
# generate anchor peer transaction
./bin/configtxgen -profile Channel2 -outputAnchorPeersUpdate ./config/CustomerOrganchors2.tx -channelID $CHANNEL_NAME2 -asOrg CustomerOrg
if [ "$?" -ne 0 ]; then
