...
func configureStub() (*MockContext, *MockStub) {
	...
	testMyAsset := new(Dong)
	testMyAsset.Name = "set value"
...
func TestMyAssetExists(t *testing.T) {
	...
	exists, err = c.ddongExists(ctx, "existingkey")
...
func TestCreateMyAsset(t *testing.T) {
	...
	// 	err = c.InitDDong(ctx, []string{"1", "statebad", "some value", "dd"})
	// 	assert.EqualError(t, err, fmt.Sprintf("Could not read from world state. %s", getStateError), "should error when exists errors")

	// 	err = c.InitDDong(ctx, []string{"1", "existingkey", "some value", "dd2"})
	// 	assert.EqualError(t, err, "The asset existingkey already exists", "should error when exists returns true")

	// 	err = c.InitDDong(ctx, []string{"missingkey", "missingkey", "some value", "dd"})
	// 	stub.AssertCalled(t, "PutState", "missingkey", []byte("{\"name\":\"missingkey\",\"id\":\"some value\",\"token\":\"dd\"}"))
	// }
	err = c.InitDDong(ctx, "1", "statebad", "some value", "dd")
	assert.EqualError(t, err, fmt.Sprintf("Could not read from world state. %s", getStateError), "should error when exists errors")

	err = c.InitDDong(ctx, "1", "existingkey", "some value", "dd2")
	assert.EqualError(t, err, "The asset existingkey already exists", "should error when exists returns true")

	err = c.InitDDong(ctx, "missingkey", "missingkey", "some value", "dd")
	stub.AssertCalled(t, "PutState", "missingkey", []byte("{\"name\":\"missingkey\",\"id\":\"some value\",\"token\":\"dd\"}"))
}
...
func TestReadMyAsset(t *testing.T) {
	...
	expectedMyAsset := new(Dong)
	expectedMyAsset.Name = "set value"
...
func TestUpdateMyAsset(t *testing.T) {
	...
	expectedMyAsset := new(Dong)
	expectedMyAsset.Name = "new value"
